...
WHAT'S NEW IN THIS VERSION (v26)
---------------------------------
Fixed the bug where signing in with Google didn't actually stay signed
in — closing the app (or the installed PWA icon) and reopening it, or
just refreshing the page, showed the "Sign in with Google" screen
again instead of going straight to the dashboard.

- ROOT CAUSE (confirmed from a screen recording): on every reopen,
  the app tried to silently refresh the Google sign-in token by
  opening a background popup automatically, before showing anything
  else. Mobile browsers (and the installed/standalone PWA in
  particular) routinely block a popup that isn't opened by a direct
  tap — the recording showed exactly this: a "Pop-up blocked" banner
  and "Sign-in failed or was cancelled", forcing a full account-
  picker + consent screen every time, even though nothing was ever
  actually logged out.
- FIX: this device already keeps a full local copy of your data for
  the Google account you last used here (that's how "Sync now" and
  offline use already worked) — the app just wasn't using it at
  startup. Now, if this device has that cached copy, it loads
  straight into your dashboard immediately, the same way the
  no-Google "local login" mode always has. The Google sync token is
  then refreshed quietly in the background afterwards; if that quiet
  refresh is ever blocked, it now just marks sync as "will retry"
  instead of bouncing you back to a sign-in screen — you keep using
  the app normally the whole time.
- Also fixed a related bug in the token-refresh code: after one
  background refresh, a later manual sign-in attempt could silently
  stop working (the refresh helper was overwriting the sign-in
  button's callback and never putting it back). Now restored
  correctly every time.
- Added a "Reconnect" button next to "Sync now" in Account & Backup,
  for the rare case where background sync keeps failing and a full
  reconnect is genuinely needed — this opens the interactive Google
  consent screen directly from your tap, which browsers always allow
  (unlike the automatic background attempt).
- Signing out (Account & Backup -> Sign out) still works exactly as
  before and is the only thing that ends a session — nothing was
  changed there.
- No new outside domains, permissions, or data collection — this is
  entirely a startup-ordering fix using data the app already stored
  on-device.
- Service worker cache bumped (APP_VERSION in sw.js) so installed
  users get this update.

WHAT'S NEW IN THIS VERSION (v25)
---------------------------------
Fixed a real bug (not a placement change this time): the Language
and Currency dropdowns in Account & Backup were silently disappearing,
leaving blank space where you tried to change them. No new outside
domains contacted, no new permissions, no new data collected, no new
files.
- FIXED: the "Language" and "Currency" `<select>` dropdowns in
  Account & Backup rendered completely empty/invisible. Cause: their
  wrapping `<label>` elements carried a `data-i18n="language"` /
  `data-i18n="currency"` attribute meant to translate the label text
  --- but the app's translation pass (`applyI18n()`) works by setting
  `element.textContent = translatedText` on every element with that
  attribute, on every screen refresh. Setting `.textContent` replaces
  *all* child content with plain text, so it was deleting the actual
  `<select>` dropdown that lived inside that same label, the moment
  after it was built and populated. Net effect: the dropdown never
  had a chance to actually appear on screen — it was destroyed within
  the same render cycle that created it, every single time, on every
  screen refresh, leaving the two blank gaps you flagged.
- Fix: moved the translated label text into its own inner `<span
  data-i18n="...">`, so `applyI18n()` only ever touches that span's
  text and never reaches into or clears the `<select>` sitting next
  to it. Both dropdowns now render and work exactly like every other
  input in the app (full-width, same style as the Email/Full name
  fields above them) — pick a language or currency and it applies
  immediately, exactly as the rest of the app already assumed it did
  (this is also what the Privacy Policy page already described).
- Safety note for this change specifically: only these two `<label>`
  lines in index.html were touched (diffed line-by-line against the
  previous version) — no JS logic, translation dictionary, or any
  other screen was touched. No eval/Function()/document.write
  anywhere, no new fetch/XHR calls, and all inline <script> blocks in
  index.html still parse cleanly with Node's parser after this
  change.
- Service worker cache bumped to v38 so installed users get this fix
  (and see the "new version ready" banner).

WHAT'S NEW BEFORE THAT (v24)
---------------------------------
Moved "Reset all data" out of the top bar and into Account & Backup,
and repurposed its old top-bar spot as a plain page-refresh button.
No new outside domains contacted, no new permissions, no new data
collected, no new files.
- MOVED: the top-bar icon next to Search/Dark-mode used to be "Reset
  all data" (the circular-arrow icon that permanently wipes every
  transaction, bill, debt, and setting). Sitting right next to
  Search — a button you tap often — made it too easy to hit by
  mistake. It now lives inside Account & Backup, in a new, clearly
  labeled "Danger Zone" card with a red-tinted border and a fuller
  warning of what it does, right below Diagnostics; it's also findable
  through the in-app search (type "reset").
- ADDED: that top-bar spot now reloads the current page instead
  (same one-tap "Refresh" action already used by the "new version is
  ready" banner) — useful if the app ever looks stuck without needing
  to dig into phone/browser settings.
- Safety note for this change specifically: only the top-bar button,
  one new settings card in the existing Account & Backup screen, and
  the in-app search index entry for it were touched (diffed line-by-
  line against the previous version) — the reset function itself
  (what it wipes, the confirmation prompt, the Google Drive sync
  afterward) is byte-for-byte the same code, just moved and rewired
  to the new button's id. No eval/Function()/document.write anywhere,
  no new fetch/XHR calls, and all inline <script> blocks in
  index.html still parse cleanly with Node's parser after this
  change.
- Service worker cache bumped to v37 so installed users get this
  update (and see the "new version ready" banner).

WHAT'S NEW BEFORE THAT (v23)
---------------------------------
Rewrote the 4-slide first-run onboarding intro (the one shown once,
before sign-in, that explains what the app does) and made it smoother
to use. No new outside domains contacted, no new permissions, no new
data collected, no new files.
- CONTENT: replaced the 4 slide headlines/descriptions with copy that
  leads with concrete benefits (expense tracking, budgets/bills/goals,
  the built-in Loan & FDR/DPS calculators, and on-device privacy)
  instead of a plain feature list — meant to help a first-time user
  actually understand what the app is for in a few seconds, and to
  put the app's most distinctive features (the loan/deposit
  calculators, full offline privacy) front and center.
- SMOOTHNESS: slides now animate in with a short, subtle
  slide+fade transition (direction-aware — forward and backward look
  different) instead of snapping instantly; this respects the OS
  "reduce motion" accessibility setting and turns animation off
  entirely for users who have that enabled.
- NAVIGATION: the progress dots under the slides are now tappable, so
  you can jump straight to any slide instead of only stepping forward
  one at a time; swipe left/right also works on touchscreens, in
  addition to the existing "Next"/"Skip" buttons.
- Safety note for this change specifically: only the onboarding
  slide content, its CSS animation rules, and its slide-switching
  functions in index.html were touched (all diffed line-by-line
  against the previous version). No sign-in, storage, backup, or any
  other screen's code was touched. No eval/Function()/document.write
  anywhere in the new code, no new fetch/XHR calls, and all inline
  <script> blocks in index.html still parse cleanly with Node's
  parser after this change.
- Service worker cache bumped to v36 so installed users get this
  update (and see the "new version ready" banner).

WHAT'S NEW BEFORE THAT (v22)
---------------------------------
Two bug fixes reported directly from screenshots — no new outside
domains contacted, no new permissions, no new data collected, no new
files.
- FIXED: the quick-add "+" button (bottom-right on every screen)
  could silently fail to open its menu. Cause: the code that closes
  the quick-add menu on an "outside click" compared the click target
  to the button element with strict equality (e.get.target!==btn).
  Tapping the button's own icon (its actual visible surface for most
  of the button) makes the click's target the icon <span> inside the
  button, not the button element itself — so that comparison read it
  as an "outside" click and closed the menu again in the very same
  tap that had just opened it. Net effect: tap the "+" button, menu
  flickers open and shut, looks broken. Fix: check whether the click
  happened anywhere inside the button (btn.contains(e.target)) instead
  of requiring an exact match on the button element — one line
  changed in index.html, nothing else touched.
- FIXED: the three tracker cards on the Goals screen (Savings /
  Emergency Fund / Future Savings) looked visibly rougher than the
  rest of the app — a cramped, non-full-width "Opening balance" input
  with its icon overlapping the wrapped label text, and plain text
  rows instead of the polished stat "chips" used everywhere else
  (Smart Monthly Targets, Smart Debt Payoff Plan, the Calculators
  screen). Root cause: those three cards were missing the ".form"
  class their input needed to pick up the app's standard full-width
  input styling, and the icon that used to sit inside the input was
  positioned with a fixed pixel offset that only lines up when the
  label text stays on one line — "Opening balance (already saved
  before tracking)" is long enough to wrap on narrower cards, so the
  icon drifted across the wrapped text instead of sitting in the
  input. Fix: the "This month" / "Total saved" figures now render as
  the same insight-chip component used across the rest of the app,
  and the Opening balance field now uses the app's standard full-
  width form-input pattern (dropping the icon that couldn't handle
  wrapping text) — same visual language, no more overlap. No element
  IDs were renamed and no calculation logic changed, so every number
  on this screen is still computed exactly as before.
- Service worker cache bumped to v35 so installed users get both
  fixes (and see the "new version ready" banner).

WHAT'S NEW BEFORE THAT (v21)
---------------------------------
A new "Calculators" screen — a Loan (EMI) calculator and a Fixed
Deposit/DPS calculator, built for any currency but with FDR/DPS
terminology and tax presets aimed at Bangladeshi users. No new
outside domains contacted, no new permissions, no new data collected,
no new files, and nothing here is saved to your backup/export data —
it's a standalone, offline planning tool, same as everything else.
- New "Calc" entry in both the desktop nav rail and the mobile side
  menu, plus in global search — takes you to two calculators:
  - Loan Calculator: enter any loan's amount, annual interest rate
    and tenure (years or months) to get the monthly EMI, total
    interest, total payment, and an optional one-time processing-fee
    line, computed with the standard reducing-balance EMI formula.
    A loan-type picker (Personal/Home/Car/Business/Education/Other)
    is for your own labeling only — it does not assume or fill in
    any interest rate, since real rates vary by bank and change over
    time.
  - Deposit Calculator: a FDR (lump-sum Fixed Deposit) mode and a
    DPS (monthly Deposit Pension Scheme) mode. FDR lets you choose
    the compounding frequency (monthly/quarterly/half-yearly/yearly);
    DPS always compounds monthly, the standard for most schemes.
    Both show total deposited, interest earned, and a net maturity
    value, with an optional Tax Deducted at Source (TDS) preset
    (None / TIN holder 10% / No TIN 15% / Custom) since banks
    withhold tax on deposit interest — all clearly marked as
    estimates to confirm with your bank, not tax advice.
  - Both calculators show an animated ring (principal/deposit vs.
    interest split), an animated growth/payoff line chart (the same
    chart style as the Smart Debt Payoff Plan), and a yearly
    breakdown table — everything recalculates live as you type or
    change any option, entirely in your browser.
- Safety note for this change specifically: plain HTML/CSS/JS added
  directly in index.html (new markup, new CSS rules under a
  clearly-commented "Calculators" section, and a self-contained set
  of new JS functions, all prefixed "calc" to avoid clashing with
  anything already in the app). No existing function, screen, or
  saved-data field was modified or renamed. All inline <script>
  blocks in index.html still parse cleanly with Node's parser after
  this change, and nothing new is fetched from, or sent to, any
  server — every number is computed on-device with plain arithmetic.
- Service worker cache bumped to v34 so installed users get this
  update (and see the "new version ready" banner).

WHAT'S NEW BEFORE THAT (v20)
---------------------------------
Two small, targeted bug fixes. No new outside domains contacted, no
new permissions, no new data collected, no new files.
- FIXED: sticky header on mobile (the header now really stays pinned
  while scrolling — verified with an actual browser engine, not just
  by reading the CSS). The top bar (logo + search/theme/refresh icons)
  was already written as "position:sticky" in the CSS, which should
  keep it pinned to the top while you scroll. The real cause turned
  out to be "overflow-x:hidden" on html/body itself (added earlier to
  stop the page from ever scrolling sideways): in real testing, having
  ANY non-"visible" overflow value directly on html/body — even just
  on the x-axis — can make some browser engines quietly turn <body>
  into its own scroll container, and once that happens a sticky
  element inside it can stop tracking the page's real scroll and
  scroll away with everything else instead of staying put. Fix:
  switched to "overflow-x:clip" instead of "overflow-x:hidden" on
  html/body. overflow-x:clip blocks sideways scrolling exactly the
  same way (verified: the page still cannot scroll horizontally at
  all), but it does not create that scroll-container side effect, so
  the sticky header now stays correctly pinned to the top through a
  full scroll, tested end-to-end in an actual Chromium browser both
  before (confirmed broken) and after (confirmed fixed) this change.
...
- Safety note for this change specifically: both fixes are one CSS
  value swap and the deletion of one fallback line inside the existing
